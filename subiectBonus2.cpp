...
public:
    ...
    // operator < pentru sortare
    bool operator<(const Serviciu3D& s) const {
        return cost < s.cost;
    }

    // operator += combina doua obiecte: aduna nrExemplare si cost
    Serviciu3D& operator+=(const Serviciu3D& s) {
...
// exemplificarea relatiei "has a" printr-o clasa de gestionare a serviciilor
class ManagerServicii {
private:
    vector<Serviciu3D> servicii;
public:
    void adaugaServiciu(const Serviciu3D& s) {
...
int main() {
    ...
    sort(lista.begin(), lista.end()); // foloseste operator <
    ...
    // STL container pentru cautarea dupa material -> map<string, vector<Serviciu3D>>
    map<string, vector<Serviciu3D>> mapMaterial;
    mapMaterial[s1.material].push_back(s1);
